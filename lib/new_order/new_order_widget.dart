...
class _NewOrderWidgetState extends State<NewOrderWidget> {
  CartRecord? newCartFromLocalState;
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      ...
      newCartFromLocalState =
          CartRecord.getDocumentFromData(cartCreateData, cartRecordReference);
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<MenuRecord>>(
      future: queryMenuRecordOnce(
        queryBuilder: (menuRecord) =>
            menuRecord.whereIn('category', choiceChipsValues.value),
      ),
      builder: (context, snapshot) {
        ...
        List<MenuRecord> newOrderMenuRecordList = snapshot.data!;
        return Title(
            title: 'NewOrder',
                            ...
                            Padding(
                              ...
                              child: FutureBuilder<List<MenuRecord>>(
                                future: queryMenuRecordOnce(
                                  queryBuilder: (menuRecord) =>
                                      menuRecord.whereIn(
                                          'category', choiceChipsValues.value),
                                ),
                                builder: (context, snapshot) {
                                  // Customize what your widget looks like when it's loading.
                                  if (!snapshot.hasData) {
                                  ...
                                  List<MenuRecord>
                                      containerFilterMenuMenuRecordList =
                                      snapshot.data!;
                                  return Container(
                                    width: MediaQuery.of(context).size.width,
                                    decoration: BoxDecoration(
                              ...
                              child: Padding(
                                ...
                                child: FFButtonWidget(
                                  onPressed: () async {
                                    ...
                                    final cartUpdateData = {
                                      'qty': FieldValue.arrayUnion(
                                          [newOrderMenuRecordList.length]),
                                    };
                                    await newCartFromLocalState!.reference
